...
let svg = d3.select("#newYorkTimeline").append("svg")
    .attr("width", width + margin.left + margin.right)
    .attr("height", height + margin.top + margin.bottom)
        .append("g")
        .attr("transform", "translate(" + margin.left + "," + margin.top + ")");

svg.append("text").attr("class","title").attr("x",width/2).attr("y",-8).text("New York COVID-19 Timeline");
...
legend.selectAll("rect")
    ...
    .attr("width", 40)
    .attr("height", 20)
legend.append("text").attr("class","legend").attr("x",width-10).attr("y",10).text("New York restriction stringency");
...
Promise.all([d3.csv("newYork/newYork_covid_stats.csv"), d3.csv("newYork/newYork_lockdown_level_timeline.csv"), d3.csv("newYork/newYork_covid_events.csv")])
    .then(function (data) {
